...
```
编码  ──► pre-commit 门禁（P2 左移）─────────────────────────────┐
          ruff check/format + pytest（scripts/pre-commit.sh）    │ 机械问题在此拦截（最便宜）
          ❌ 违规 → 拦截，修复后再提交                            │
          ✔ 通过 ───────────────────────────────────────────────┘
               │
               ▼
          提交 commit ──► CI 运行 ──► 审查层（主动预防，抢在 CI 前）
               │                       │
               │                       ├─ 机械改动        → 不派 agent，靠门禁
               │                       ├─ 语义/新功能/重构 → 阶段 A + B 全跑（语义核对面）
               │                       └─ 架构/记忆/安全   → 阶段 A + B + 人工把关清单
               │
               ▼  （若审查/门禁全过）
          CI 正常通过 ──► 结束
               │
               ▼ （若 CI 仍失败 → 漏网之鱼）
          修复层 self-heal（被动救火，最后防线）
              if: failure() → codex（danger-full-access）
                  ├─ 机械问题 → ruff --fix 快路径（不拉 codex）
                  └─ 语义残留 → codex 修复 → ruff 复检 → push auto-heal 分支 → gh pr create
              人工 review 后合并 ──► 回到 main
```

- **审查层**（编码后）：**主动预防，左移**，抢在 CI 之前抓语义/架构问题
- **修复层**（CI 失败后）：**被动兜底，后置**，只救漏网之鱼
- **门禁**（commit 前）：最便宜的第一道闸，机械问题在此清零
